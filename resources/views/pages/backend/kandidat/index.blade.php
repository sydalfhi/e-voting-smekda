    ...
    <div class="h-screen col-span-10 xl:col-span-8 ">
        ...
        <div class="h-screen p-5 md:p-10 ">
            ...
            <div class="grid w-full grid-cols-1 py-5 bg-red md:grid-cols-2 xl:grid-cols-3 md:gap-x-5">
                ...
                <div class="w-full border shadow-lg border-separate border-[#272727]/10 rounded  p-3">
                    <div class="flex flex-col  justify-around">

                        <h1 class="text-3xl font-bold">{{ $item->kandidat->nomer }}</h1>

                        <div class="bg-[#272727] grid place-items-center rounded-sm ">
                            <img src="{{ asset('assets/kandidat/' . $item->kandidat->poster) }}" alt="avatar" class=" object-cover block w-[200px] bg-[#272727] aspect-square">
                        </div>
                    ...
                    <div class="mt-2">
                        ...
                        <p class="text-[13px] text-slate-600 min-h-[80px] ">{{ $item->visi }}</p>
                        <p class="mt-3 text-xl font-bold uppercase">misi</p>
                        <p>
                        <ul class="pl-5 list-decimal text-[13px] text-slate-600 min-h-[150px] ">
                        ...
                        <div class="grid grid-cols-2 mt-3 gap-x-3">
                            <a href="{{ route('kandidat.edit', $item->kandidat->id) }}" class="block py-2 font-bold text-center text-white bg-blue-500 rounded">
                                Edit
                            </a>

                            <form class="block py-2 font-bold text-center text-white rounded bg-rose-500" action="{{ route('kandidat.destroy', $item->kandidat->id) }}" method="post">
                                @csrf
                                @method('delete')
                ...
                @empty
                <h1 class="text-xl text-left capitalize w-full col-span-4  px-5 py-10 rounded-lg bg-gray-200 text-[#272727] ">
